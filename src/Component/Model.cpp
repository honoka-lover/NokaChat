...
void Model::loadModel(string const& path)
{
    ...
    const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace | aiProcess_GenNormals);
    // check for errors
...
vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {   // if texture hasn't been loaded already, load it
            Texture texture;
            std::string pt = this->directory + "/" + std::string(str.C_Str());
            texture.id = TextureFromFile(pt.data(), this->directory);
...
void Mesh::Draw(QOpenGLShaderProgram* program)
{
    ...
    for (unsigned int i = 0; i < textures.size(); i++)
    {
        ...
        program->setUniformValue((name + number).c_str(), i);
